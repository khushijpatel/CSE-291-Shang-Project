...
class FLANPoliticalQA:
    def __init__(self, model_name='google/flan-t5-base', device='cuda:1' if torch.cuda.is_available() else 'cpu'):
        self.device = device
    ...
    def answer_question(self, question, max_length=3000, temperature=0.85, num_return_sequences=1):
        ...
        input_text = f"Answer the following political question: {question}"
...
def main():
    ...
    political_qa.train(
        ...
        batch_size=16,
        epochs=5,
        output_dir='political_flan_model'
    )
    ...
    results_df.to_csv('flan_results.csv', index=False)
    print(f"\nResults saved to flan_results.csv")
