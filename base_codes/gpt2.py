...
class GPT2PoliticalQA:
    def __init__(self, model_name='gpt2', device='cuda:1' if torch.cuda.is_available() else 'cpu'):
        self.device = device
...
def main():
    ...
    political_qa.train(
        ...
        batch_size=16,
        epochs=5,
        output_dir='political_gpt2_model'
    )
    ...
    results_df.to_csv('gpt_results.csv', index=False)
    print(f"\nResults saved to gpt_results.csv")
